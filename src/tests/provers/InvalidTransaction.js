...
module.exports = test('InvalidTransaction', async t => { try {
  ...
  async function state (opts = {}) {
    ...
    if (opts.fraud === 'outputs-size') {
      outputs = ['0x030003aabb']; // second 3 is invalid here, should be 2
    }
    ...
    if (opts.fraud === 'output-shift-mod') {
      const invalidOutput = tx.OutputTransfer({
        token: '0x01',
        amount: 0,
        owner: producer,
      });
      invalidOutput.properties.shift().set(3);
    ...
    if (opts.fraud === 'outputs-amount-overflow' && opts.name == 'with-mod') {
      const invalidOutput = tx.OutputTransfer({
        token: '0x01',
        amount: 0,
        owner: producer,
    ...
    if (opts.fraud === 'outputs-amount-overflow' && opts.name == 'no-mod') {
      const invalidOutput = tx.OutputTransfer({
        token: '0x01',
        amount: 0,
        owner: producer,
      });
      invalidOutput.properties.shift().set(0); // no mod
      invalidOutput.properties.amount().set(utils.emptyBytes32 + '00');
      outputs = [invalidOutput];
    }

    if (opts.fraud === 'outputs-amount-overflow' && opts.name == 'mod-only') {
      const invalidOutput = tx.OutputTransfer({
        token: '0x01',
        amount: 45,
        owner: producer,
      });
      invalidOutput.properties.shift().set(8); // mod pushes past 32
      invalidOutput.properties.amount().set(utils.emptyBytes32);
      outputs = [invalidOutput];
    }
    ...
    if (opts.fraud === 'witness-caller-overflow') {
      const invalidOutput = protocol.witness.Caller({
        blockNumber: (await t.provider.getBlockNumber()) + 2, // one for root one for block = 2
        owner: producer, 
      });
      witnesses = [invalidOutput];
    }
    ...
    if (opts.fraud === 'outputs-amount-shift-overflow') {
      const invalidOutput = tx.OutputHTLC({
        token: '0x01',
        amount: '0x',
        owner: producer,
      ...
      invalidOutput.properties.shift().set('0xFF'); // theory check, not possible.
      outputs = [invalidOutput];
    }
    ...
    if (opts.fraud === 'transaction-size-overflow') {
      outputs = [
        tx.OutputReturn({
          data: utils.hexZeroPad('0xaa', 512),
        }),
        tx.OutputReturn({
          data: utils.hexZeroPad('0xaa', 350),
        }),
      ];
    }
    ...
    if (opts.fraud === 'outputs-index-overflow') {
      outputs = [
        tx.OutputReturn({
          data: '0xaa',
        }),
    ...
    // no fraud
    const result = await t.wait(contract.proveInvalidTransaction(proof.encodePacked(), {
      ...overrides,
  ...
  await state ({ useErc20: true, tinyReturns: true });
  ...
  await state ({ useErc20: true, fraud: 'outputs-amount-overflow', name: 'mod-only' });
  await state ({ useErc20: true, fraud: 'outputs-amount-overflow', name: 'no-mod' });
  await state ({ useErc20: true, fraud: 'outputs-amount-overflow', name: 'with-mod' });
  ...
  await state ({ useErc20: true, fraud: 'outputs-size' });
  ...
  await state ({ useErc20: true, fraud: 'witness-caller-overflow' });
  await state ({ useErc20: true, fraud: 'outputs-index-overflow' });
  await state ({ useErc20: true, fraud: 'transaction-size-overflow' });

  // Formal verification checks, unreachable.
  // 'metadata-index-underflow',
  // 'metadata-deposit-token-underflow',
  // 'metadata-root-underflow',
  // 'metadata-output-underflow',
  // 'inputs-type-underflow'
  // 'outputs-amount-shift-underflow'
  // 'inputs-witness-reference-underflow'
  // 'owner-length-underflow'
  // 'output-shift-mod'
  // 'outputs-amount-shift-overflow'
