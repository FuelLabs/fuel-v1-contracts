...
module.exports = test('InvalidInput', async t => { try {
  ...
  async function state (opts = {}) {
    ...
    if (opts.revert === "output-index-mismatch") {
      metadata = [tx.Metadata({
        blockHeight: 1,
        rootIndex: 0,
        transactionIndex: 0,
  ...
  await state ({ useErc20: false, deposit: true, fraud: "input-deposit-value-underflow" });

  await state ({ useErc20: false, root: true });

  await state ({ useErc20: false, revert: "block-height-mismatch" });
  await state ({ useErc20: false, revert: 'output-index-mismatch' });
