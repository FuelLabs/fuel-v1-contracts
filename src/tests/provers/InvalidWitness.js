...
module.exports = test('InvalidWitness', async t => {
    ...
    async function produceTransaction(utxo = {}, opts = {}) {
        ...
        if (opts.useHTLCExpiredInput) {
            const inputUtxo = protocol.outputs.UTXO({
                outputType: protocol.outputs.OutputTypes.HTLC,
                owner: t.wallets[1].address, // <-- testing this
                amount: utils.parseEther('0.1'),
                expiry: 500000000,
            ...
            data = [
                inputUtxo.keccak256(),
                inputUtxo.keccak256(), // <-- fake but okay
                inputUtxo.keccak256(),
                inputUtxo.keccak256(),
            ];
            metadata = [
            ...
            inputs = [
                protocol.inputs.InputHTLC({
                    preImage: utils.keccak256('0xaa'),
                    witnessReference: opts.invalidWitnessIndex
                        ? 2
                        : 7, // <-- try inner witness recovery
                }),
            ...
            // Set the first proof.s
            proofs = inputUtxo.encode();
        }

        // Will currupt the last byte of last hash.
        if (opts.curruptLastDataHash) {
            const lastHash = data[data.length - 1].slice(0, -2);
            opts.proofData = data.concat([]);
            opts.proofData[data.length - 1] = lastHash + 'FF';
        }

        // The inputs and metadata now matter, for complexity sake we will do a root.
        const transactionData = {
            override,
            witnesses: witnesses || opts.witnesses || [ t.wallets[0] ],
            metadata,
            data, // The root data.
            inputs,
            signatureFeeToken: 0,
    ...
    async function state (opts = {}) {
        ...
        // Invalidate a caller witness.
        if (opts.invalidateCaller) {
            // Get the current witness.
            const witness = transaction.properties.witnesses().get();
            const invalidWitness = witness.slice(0, -1).concat([
                '0xFF',
            ]);
        ...
        // Invalidate a caller witness.
        if (opts.invalidateProducer) {
            // Get the current witness.
            const witness = transaction.properties.witnesses().get();
            const invalidWitness = witness.slice(0, -10).concat([
                '0xFF',
                '0xFF',
                '0xFF',
                '0xFF',
        ...
        if (opts.fraud) {
            ...
            if (!result.events.length) {
                console.log(result);
                t.ok(0, 'no fraud events, likely reverted');
                return;
            }
    ...
    await state({
        inputs: [
            ...
            await produceTransaction(protocol.outputs.UTXO({
                ...
                owner: '0x000000', // <-- invalid null registered
                witnessReference: 2,
            }),
        ],
        witnesses: [
            t.wallets[0],
            t.wallets[0],
            { _caller: true }, // <-- is registered
        ],
        inputOutputIndex: 7,
        fraud: 'utxo-witness',
        name: 'not registered witness, registered null address case complex registered',
    });
    ...
    await state({
        ...
        witnesses: [
            t.wallets[0], 
            t.wallets[0], 
            t.wallets[4], // <-- not registered
        ],
        inputOutputIndex: 7,
        fraud: 'utxo-witness',
        name: 'not registered witness, registered null address case complex',
    });
    ...
    await state({
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                ...
                returnOwner: '0x00', // <-- use registered null address
            }),
        ],
        witnesses: [
            t.wallets[4], // <-- not registered
        ...
        name: 'not registered witness, invalid witness null owner registered htlc return',
    });
    ...
    await state({
        ...
        inputOutputIndex: 0,
        fraud: 'utxo-witness',
        name: 'not registered witness, registered null address case',
    });
    ...
    await state({
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                ...
                owner: '0x00', // <-- use registered null address
            }),
        ],
        witnesses: [
            t.wallets[4], // <-- not registered
        ...
        inputOutputIndex: 0,
        fraud: 'htlc-owner',
        name: 'not registered witness, invalid witness null owner registered htlc',
    });
    ...
    await state({
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                outputType: protocol.outputs.OutputTypes.HTLC,
                owner: producer, // <-- testing this
                expiry: 50000000,
                returnOwner: utils.emptyAddress,
            }), {
                owner: '0x01', // <-- use registered
            }),
        ],
        witnesses: [
            t.wallets[4], // <-- not registered
        ...
        inputOutputIndex: 0,
        fraud: 'htlc-owner',
        name: 'not registered witness, invalid witness normal case',
    });
    ...
    await state({
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                ...
                useHTLCInput: true,
                returnOwner: '0x00', // <-- testing this null-address
                witnessReference: 0,
                inputUTXOOverride: {
                    owner: producer,
                    expiry: 0,
        ...
        name: 'not registered witness, revert on invalid inner proof null witness',
    });
    ...
    await state({
        ...
        valid: 'test registered committed addresses',
    });
        ...
        inputs: [
            ...
            await produceTransaction(protocol.outputs.UTXO({
                ...
                inputUTXOOverride: {
                    owner: producer,
                    expiry: 0,
                    returnOwner: utils.emptyAddresss, // <-- testing this
                },
                returnOwner: '0x000001',
            }),
        ],
        revert: 'htlc-witness-signature',
        inputOutputIndex: 7,
        valid: 'invalid return owner address for registered proof',
    });
    ...
    await state({
        ...
        witnesses: [
          t.wallets[1], // <-- this is invalid 
        ],
        fraud: 'deposit-witness',
        name: 'invalid owner resolved',
    });

    // Go through each witness, check validity.
    for (let outputInputIndex = 0; outputInputIndex < 4; outputInputIndex++) {
        await state({
            inputs: [
                await produceTransaction(protocol.outputs.UTXO({
            ...
            inputOutputIndex: outputInputIndex,
            valid: 'many different valid witnesses',
        });
    }
    ...
    await (async () => {
        ...
        await state({
            ...
            inputOutputIndex: 7,
            valid: 'invalid input 7 selects invalid wit 0',
        });
    ...
    await (async () => {
        ...
        const otherDeposit = protocol.deposit.Deposit({
            ...deposit.object(),
        });
        otherDeposit.witnessReference = 0;
        ...
        await state({
            inputs: [
                ...
                await produceTransaction(protocol.outputs.UTXO({
                    ...
                    witnessReference: 2,
                    do8DepositInputs: true,
                    curruptLastDataHash: true, // <-- currupts last hash
                }),
            ],
            witnesses: [
                t.wallets[0],
            ...
            inputOutputIndex: 7, // <-- this is invalid
            revert: 'deposit-witness-signature',
            name: 'input provided at last proof currupt last data hash',
        });
        ...
        await state({
            inputs: [
                ...
                await produceTransaction(protocol.outputs.UTXO({
                    ...
                    useDepositInput: true,
                    witnessReference: 1, // <-- this is invalid
                }),
            ],
            witnesses: [
                t.wallets[0],
            ...
            inputOutputIndex: 7, // <-- this is invalid
            fraud: 'utxo-witness',
            valid: 'invalid input 7 selects invalid wit 1',
        });
    ...
    await (async () => {
        ...
        await state({
            ...
            witnesses: [
                ...
                t.wallets[1], // <-- this is invalid
                t.wallets[1], // <-- this is invalid
                t.wallets[2], // <-- this is invalid
            ],
            inputOutputIndex: 7, // <-- this is invalid
            valid: 'valid input 7 selects valid witness 0',
        });
    ...
    await (async () => {
        ...
        otherDeposit.witnessReference = 1;
    ...
    await (async () => {
        ...
        otherDeposit.witnessReference = 7;
        ...
        await state({
            ...
            inputOutputIndex: 7,
            fraud: 'deposit-witness',
            valid: 'invalid input 7 selects invalid wit 7',
        });
    ...
    await (async () => {
        ...
        await state({
            ...
            inputOutputIndex: 7,
            valid: 'invalid but valid selected',
        });
    ...
    await state({
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                owner: utils.emptyAddress, // <-- should never be able to sign as null.
            }), { useDepositInput: true }),
        ],
        useDepositInput: true,
        nullifyWitness: true, // <-- this is invalid.
        fraud: 'null-witness',
        name: 'invalid null witness check',
    });
    ...
    await state({
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                owner: t.wallets[3].address, // <-- this is invalid
            }), {
                owner: '0x02',
                addressA: t.wallets[3].address, // <-- this is invalid
            }),
        ],
        fraud: 'utxo-witness',
        name: 'invalid owner resolved',
    ...
    await state({
        witnesses: [ { _caller: true }, t.wallets[1] ], // <-- this is invalid
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                outputType: protocol.outputs.OutputTypes.HTLC,
                owner: utils.emptyAddress,
                expiry: 3, // will be expired.
                returnOwner: producer, // will use this address
            }), {
                witnessReference: 1,
                returnOwner: '0x01', // use producer
            }),
        ],
        fraud: 'htlc-owner-return',
        name: 'committed htlc return address invalid',
    });
    ...
    await state({
        ...
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                ...
                owner: utils.emptyAddress,
                expiry: 3, // will be expired.
                returnOwner: producerFunnel, // will use this address
            }), {
                witnessReference: 1,
            }),
        ],
    ...
    await state({
        ...
        inputs: [
            await produceTransaction(protocol.outputs.UTXO({
                outputType: protocol.outputs.OutputTypes.HTLC,
                owner: producer, // this will be used, not expired.
                expiry: 3000000,
        ...
        fraud: 'htlc-owner',
        name: 'committed htlc owner address invalid',
    });
    ...
    await state({
        ...
        inputs: [
            ...
            block3.rootTransactionProof,
            await produceTransaction(protocol.outputs.UTXO({
                owner: producer,
            })),
            deposit,
            await produceTransaction(protocol.outputs.UTXO({
                owner: t.wallets[1].address, // <--- this is invalid
            })),
        ],
        inputOutputIndex: 7,
        fraud: 'utxo-witness',
    ...
    // Formal checks:
    // 'witness-position-overflow', revert overflow checks.
    // 'input-position-overflow', revert overflow checks.
    // 'output-position-overflow', revert overflow checks.
