// Some complex invalidTransaction.
...
module.exports = test('InvalidTransaction_complex', async t => { try {
  ...
  async function state (opts = {}) {
    ...
    if (opts.fraud === 'root-height-overflow') {
      metadata = [tx.Metadata({
        blockHeight: 2, // This must be below 1.
        rootIndex: 0,
        transactionIndex: 0,
        outputIndex: 0,
      })];
    ...
    if (opts.fraud === 'root-transaction-index') {
      const invalidInput = tx.InputRoot({});
      inputs = [invalidInput];
      metadata = [tx.Metadata({
        blockHeight: 1,
        rootIndex: 1,
    ...
    if (opts.fraud === 'root-output-index') {
      const invalidInput = tx.InputRoot({});
      inputs = [invalidInput];
      metadata = [tx.Metadata({
        blockHeight: 1,
    ...
    let txs = opts.txs || [transaction];

    if (opts.fraud === 'transaction-size-underflow') {
        transaction = tx._Transaction({
            witnesses: '0x02' + utils.hexlify(utils.randomBytes(32)).slice(2),
            metadata: ['0x0000000100000000'],
            inputs: '0x0000',
            outputs: '0x03000100', // return Data.
        });
        transaction.properties.length().set(utils.hexDataLength(transaction.encodePacked()) - 2);
        txs = [transaction].concat(...txs);
    }
    ...
    await t.wait(contract.commitRoot(root2.properties.merkleTreeRoot().get(), 0, 1, combine(txs), overrides),
      'valid submit', errors);
    ...
    await t.wait(contract.commitRoot(root2.properties.merkleTreeRoot().get(), 0, 3, combine(txs), overrides),
        'valid submit', errors);
    ...
    const header2 = (new BlockHeader({
      ...
      numTokens,
      numAddresses: 1,
      roots: [root3.keccak256Packed(), root2.keccak256Packed()],
    }));

    const currentBlock2 = await t.provider.getBlockNumber();
    const currentBlockHash2 = (await t.provider.getBlock(currentBlock2)).hash;
    const block2 = await t.wait(contract.commitBlock(currentBlock2, currentBlockHash2, 2, [root3.keccak256Packed(), root2.keccak256Packed()], {
      ...overrides,
      value: await contract.BOND_SIZE(),
    }), 'commit block', errors);
  ...
  await state ({ useErc20: true, fraud: 'root-height-overflow' });

  await state ({ useErc20: true, fraud: 'root-transaction-index' });
  await state ({ useErc20: true, fraud: 'root-output-index' });

  // Formal verification checks, unreachable.
  // 'metadata-index-underflow',
  // 'transaction-size-underflow', other length checks should make this unreachable.
  //    -- note other length checks are done first b/c use of less ops.
