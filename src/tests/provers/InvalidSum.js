...
module.exports = test('InvalidSum', async t => {
    ...
    const userA = t.wallets[1].address;
    ...
    const funnelA = await contract.funnel(userA);
    const depositValue = utils.bigNumberify(1000);

    // Do ERC20 z.
    await t.wait(erc20.transfer(funnel, depositValue, overrides), 'erc20 transfer');
    await t.wait(erc20.transfer(funnelA, depositValue, overrides), 'erc20 transfer');
    ...
    // Sign with Funnel.
    const depositA = new protocol.deposit.Deposit({
        token: tokenId,
        owner: userA,
        blockNumber: utils.bigNumberify(await t.getBlockNumber()).add(1),
        value: depositValue,
    ...
    await t.wait(
        contract.deposit(userA, token, overrides),
        'token deposit',
        errors,
    );
    ...
    async function state (opts = {}) {
        ...
        // The data to go to proofing.
        let proofData = opts.proofData
            || opts.data
            || inputProofHashes(opts.inputs);

        // Will currupt the last byte of last hash.
        if (opts.curruptLastDataHash) {
            const lastHash = proofData[proofData.length - 1].slice(0, -10);
            proofData[proofData.length - 1] = lastHash + 'FFFFFFFFFF';
        }
    ...
    await state ({
        ...
        signatureFee: utils.parseEther('0.0'),
        outputs: [
    ...
    await state ({
        ...
        signatureFeeToken: 1,
        signatureFee: utils.parseEther('20303'),
        outputs: [
            protocol.outputs.OutputTransfer({
                noshift: true,
                amount: utils.parseEther('0'),
    ...
    await state ({
        ...
        curruptLastDataHash: true, // <--- will currupt the last hash 1 byte
        revert: 'htlc-witness-signature',
        token: utils.emptyAddress,
        name: 'all data is correct except last hash',
    });
    ...
    await state ({
        ...
        signatureFee: utils.parseEther('0'),
        rootFee: utils.parseEther('0'),
        outputs: [
            protocol.outputs.OutputTransfer({
                noshift: true,
        ...
        token: utils.emptyAddress,
        valid: 'normal tx with normal witness',
    });
    ...
    await state ({
        ...
        token: utils.emptyAddress,
        valid: 'normal tx with caller witness',
    });
    ...
    await state ({
        ...
        token: utils.emptyAddress,
        valid: 'normal tx with producer witness',
    });

    await state ({
        witnesses: [ t.wallets[1] ],
        inputs: [
            depositA,
        ],
        signatureFee: utils.parseEther('0'),
        ...
        outputs: [
            protocol.outputs.OutputTransfer({
                noshift: true,
                amount: depositA.properties.value().get(),
                token: 1,
                owner: producer,
            }),
        ],
        token: utils.emptyAddress,
        valid: 'normal tx other witness A',
    });
    ...
    await state ({
        ...
        invalidateSignature: true, // <-- invalidate signature
        token: utils.emptyAddress,
        revert: "deposit-witness-signature",
        name: 'invalidate the witness data',
    });
    ...
    await state ({
        ...
        data: [ // <-- invalid wrong count
            utils.keccak256('0xaa'),
            utils.keccak256('0xaa'),
        ],
        revert: 'data-length-alignment',
        token: utils.emptyAddress,
        name: 'wrong number of data hashes provided',
    });
    ...
    await state ({
        ...
        revert: 'htlc-witness-signature',
        token: utils.emptyAddress,
        name: 'htlc return owner caller funnel with invalid null witness',
    });
    ...
    await state ({
        ...
        outputs: [
            protocol.outputs.OutputTransfer({
                noshift: true,
                amount: utils.parseEther('0.44'), // <- this is invalid.
                token: 0,
                owner: producer,
            }),
        ],
    ...
    await state ({
        inputs: [
            ...
            overflowBlock.rootTransactionProof, // <-- this is invalid.
        ],
        outputs: [
            protocol.outputs.OutputTransfer({
                amount: utils.bigNumberify(0), // <-- this is invalid.
                token: tokenId,
                owner: producer,
            }),
            protocol.outputs.OutputTransfer({
                amount: deposit.properties.value().get(), // <-- this is invalid.
                token: tokenId,
                owner: producer,
            }),
        ],
        token,
        fraud: 'summing-mul-overflow',
    });
    ...
    await state ({
        ...
        inputs: [
            ...
            (() => {
                let input = protocol.outputs.UTXO({
                    outputIndex: 0,
                    outputType: 2,
                    amount: utils.parseEther('1').add(1), // <---- this is invalid.
                    owner: producer,
                    token: 0, // <--- it's the ether token
                    digest: utils.randomBytes(32),
                    expiry: 3,
                    returnOwner: utils.emptyAddress,
                });
        ...
        token: utils.emptyAddress, // ether.
        fraud: 'sum',
        name: 'very complex summing tx',
    });
    ...
    await state ({
        ...
        outputs: [
            protocol.outputs.OutputTransfer({
                amount: utils.bigNumberify(maxNum), // <-- this is invalid.
                token: tokenId,
                owner: producer,
            }),
            protocol.outputs.OutputTransfer({
