...
module.exports = test('Accounting', async t => {
    ...
    t.equalBig(await erc20.balanceOf(producer),
        preERC20Balance.add(deposit.object().value),
        'balance after withdraw in Fuel');

    // withdraw bond header.
    await t.wait(
        contract.bondWithdraw(block1.block.encodePacked(), overrides),
        'withdraw bond', errors);
    
    t.equalBig(await t.getProvider().getBalance(contract.address), utils.parseEther('1.0'), 'ether post block withdraw 1');

    await t.wait(
        contract.bondWithdraw(block2.block.encodePacked(), overrides),
        'withdraw bond', errors);

    t.equalBig(await t.getProvider().getBalance(contract.address), utils.parseEther('0.0'), 'ether post block withdraw 2');
