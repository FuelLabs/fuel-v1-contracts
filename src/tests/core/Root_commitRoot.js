...
module.exports = test('Root_commitRoot', async t => { try {
    ...
    // Overflow check.
    await t.revert(contract.commitRoot(merkleRootA, 1, 0, emptyTxs, overrides),
      errors['token-overflow'], 'token-overflow');
    ...
    const underflowBytes = utils.randomBytes(43);
    await t.revert(contract.commitRoot(merkleRootA, 0, 0, underflowBytes, overrides),
      errors["root-size-underflow"], "root-size-underflow");
    ...
    // Commit root overflow.
    await t.revert(t.wallets[0].sendTransaction({
      ...overrides,
      to: contract.address,
      data: contract.interface.functions.commitRoot.encode([
        merkleRootA,
        0,
        0,
        emptyTxs,
      ]) + utils.hexlify(utils.randomBytes(32000)).slice(2),
    }),
    errors['calldata-size-overflow'], 'calldata-size-overflow');
    ...
    // Unreachable: 'token-underflow' -- theory check.
    // Unreachable: 'root-index-underflow' -- theory check.
    // 'origin-not-caller'
