...
module.exports = test('Block_commitBlock', async t => { try {
    ...
    const overflowRoots = (new Array(129)).fill(0).map(v => utils.randomBytes(32));
    ...
    await t.revert(contract.commitBlock(3000000, currentBlockHash, 1, [croot], {
      ...overrides,
      value: await contract.BOND_SIZE(),
    }), errors['minimum-block-number'], 'minimum-block-number', errors);

    // Check min block hash.
    await t.revert(contract.commitBlock(0, currentBlockHash, 1, [croot], {
      ...overrides,
      value: await contract.BOND_SIZE(),
    }), errors['minimum-block-hash'], 'minimum-block-hash', errors);

    // Formal Unreachable Checks:
    // 'block-height-underflow'
