...
const block = require('@fuel-js/protocol2/src/block');
const { defaults } = require('../utils/harness.js');

module.exports = test('Fuel_constructor', async t => { try {

  t.ok(utils.hexDataLength(bytecode) < 24576, 'contract-bytecode-size-check');
  console.log('Fuel size', utils.hexDataLength(bytecode));
  ...
  try {
    ...
    t.equal(typeof error, "object", "name-length");
  }
  ...
  try {
    ...
    t.equal(typeof error, "object", "version-length");
  }

  try {
    await t.deploy(abi, bytecode, [
      producerA,
      20,
      20,
      20,
    ...
    t.equal(typeof error, "object", "bond-divisible");
  }
