...
module.exports = test('Data', async t => {
    ...
    async function state (opts = {}) {
        ...
        let inputs = inputsFromProofs(opts.inputs);

        // Currupt the last type.
        if (opts.overrideLastInputType) {
            // Inputs encodePacked.
            inputs = inputs.map(v => v.encodePacked());

            // Select last input.
            const lastInput = inputs[inputs.length - 1];

            // Currupt last input type. 
            inputs[inputs.length - 1] = '0x04' + lastInput.slice(4);
        }
    ...
    await state ({
        ...
        overrideLastInputType: true, // <-- currupt the last type
        name: 'override-last-type',
    ...
    await state ({
        ...
        name: 'invalid data length provided',
        token: utils.emptyAddress,
    });
