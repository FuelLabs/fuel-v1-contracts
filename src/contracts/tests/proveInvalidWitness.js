...
module.exports = test('proveInvalidWitness', async t => { try {
  ...
  async function state (opts = {}) {
    ...
    let specifiedOutputs = [ tx.OutputUTXO({
      amount: 100,
      token: tokenId,
      owner,
    }), tx.OutputWithdraw({
    ...
    let returnOwner = (opts.fraud === "htlc-owner-return"
      ? utils.emptyAddress
      : producer);
    ...
    if (opts.htlc) {
      specifiedOutputs = [ tx.OutputHTLC({
        ...
        returnOwner: opts.commitAddress ? (opts.funnel ? owner : producer) : owner,
      }) ];
    }
    ...
    let utxo = new tx.UTXO({
      ...
      amount: 100,
      token: tokenId,
      owner: opts.commitAddress ? (opts.funnel ? outputOwner : producer) : outputOwner,
      expiry: opts.htlc ? (opts.fraud === "htlc-owner-return" ? 0 : 50000) : 0,
      digest: opts.htlc ? utils.emptyBytes32 : utils.emptyBytes32,
      returnOwner: opts.htlc ?
        (opts.commitAddress ? (opts.funnel ? outputOwner : producer) : outputOwner)
        : utils.emptyAddress,
    });
    ...
    if (opts.caller) {
      const callerTx = await contract.commitWitness(transactionB.transactionHashId(), {
        ...overrides,
      });
      ...
      const callerTx2 = await otherContract.commitWitness(transactionB.transactionHashId(), {
        ...overrides,
      });
    ...
    let proof = tx.TransactionProof({
      ...
      indexes: { output: 0 },
      transactionIndex: 1,
      token: opts.commitAddress ? (opts.funnel ? outputOwner : producer) : outputOwner,
      selector: opts.commitAddress ? (opts.funnel ? outputOwner : producer) : outputOwner,
    });
    ...
    if (opts.root) {
      ...
      data = [ opts.revert === "root-data" ? utils.emptyBytes32 : root.keccak256Packed() ];
      ...
      if (opts.caller) {
        const callerTx3 = await contract.commitWitness(transactionB.transactionHashId(), {
          ...overrides,
        });
      ...
      await t.wait(contract.commitRoot(root2.properties.merkleTreeRoot.get(), 0, 0, combine(txs2), overrides),
        'valid submit', errors);
      await t.wait(contract.commitBlock(0, 2, [root2.keccak256Packed()], {
        ...overrides,
        value: await contract.BOND_SIZE(),
    ...
    if (opts.fraud) {
      const fraudTx = await t.wait(contract.proveInvalidWitness(proof.encodePacked(), inputs, {
        ...overrides,
    ...
    if (opts.revert) {
      await t.revert(contract.proveInvalidWitness(proof.encodePacked(), inputs, {
        ...overrides,
    ...
    await t.wait(contract.proveInvalidWitness(proof.encodePacked(), inputs, {
      ...overrides,
    }), 'no fraud', errors);
    t.equalBig(await contract.blockTip(), opts.root ? 2 : 1, 'tip');
  }
  ...
  await state ({ useErc20: true, commitAddress: true, caller: true, htlc: true });
  await state ({ useErc20: true, commitAddress: true, producer: true, htlc: true });
  await state ({ useErc20: true, commitAddress: true, fraud: "utxo-witness" });
  await state ({ useErc20: true, commitAddress: true, root: true, fraud: "root-witness" });
  await state ({ useErc20: true, commitAddress: true, htlc: true, fraud: "htlc-owner-return" });
  await state ({ useErc20: true, commitAddress: true, htlc: true, fraud: "htlc-owner" });
  ...
  await state ({ useErc20: true, caller: true, root: true, fraud: "root-witness" });
  await state ({ useErc20: true, caller: true, htlc: true, fraud: "htlc-owner-return" });
  await state ({ useErc20: true, caller: true, htlc: true, fraud: "htlc-owner" });

  await state ({ useErc20: true, funnel: true, caller: true, fraud: "utxo-witness" });
  await state ({ useErc20: true, funnel: true, caller: true, root: true, fraud: "root-witness" });
  await state ({ useErc20: true, funnel: true, caller: true, htlc: true, fraud: "htlc-owner-return" });
  await state ({ useErc20: true, funnel: true, caller: true, htlc: true, fraud: "htlc-owner" });

  await state ({ useErc20: true, producer: true, fraud: "utxo-witness" });
  await state ({ useErc20: true, producer: true, root: true, fraud: "root-witness" });
  await state ({ useErc20: true, producer: true, htlc: true, fraud: "htlc-owner-return" });
  await state ({ useErc20: true, producer: true, htlc: true, fraud: "htlc-owner" });
